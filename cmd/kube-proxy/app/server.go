...
func (s *ProxyServerConfig) AddFlags(fs *pflag.FlagSet) {
	...
	fs.StringVar(&s.ProxyMode, "proxy-mode", "haproxy", "Which proxy mode to use: 'userspace' (older, stable) , 'iptables' (experimental) or haproxy (for hyper only). If blank, look at the Node object on the Kubernetes API and respect the '"+experimentalProxyModeAnnotation+"' annotation if provided.  Otherwise use the best-available proxy (currently userspace, but may change in future versions).  If the iptables proxy is selected, regardless of how, but the system's kernel or iptables versions are insufficient, this always falls back to the userspace proxy.")
...
func NewProxyServerDefault(config *ProxyServerConfig) (*ProxyServer, error) {
	...
	case proxyModeUserspace:
		...
		proxierUserspace, err := userspace.NewProxier(loadBalancer, config.BindAddress, iptInterface, config.PortRange, config.IptablesSyncPeriod, config.UDPIdleTimeout, client, false)
		if err != nil {
			glog.Fatalf("Unable to create proxier: %v", err)
		}
